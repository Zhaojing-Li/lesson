...
async def list_resources() -> list[Resource]:
    ...
    try:
        ...
        # 此处应使用普通的 with，而不能使用 async with。
        # 理由如下：
        # 1. mysql-connector-python（或类似的同步 MySQL 驱动）中的 connect 返回的是同步连接对象，不支持异步上下文管理协议（即没有 __aenter__ 和 __aexit__ 方法）。
        # 2. 只有基于异步驱动（如 aiomysql、asyncmy 等）返回的连接对象才可以用 async with。
        # 3. 如果强行 async with，会报错：'MySQLConnection' object does not support the asynchronous context manager protocol。
    ...
    async with stdio_server() as (read_stream, write_stream):
        try:
            ...
            await mcp.run(
                ...
                # mcp.create_initialization_options() 方法用于生成服务器初始化时需要传递给客户端的配置信息和上下文信息。
                # 配置信息通常包括服务器名称、版本、支持的功能（capabilities）、说明文档等；
                # 上下文信息可以包含服务器的自定义说明、实验性能力、通知选项等。
                # 例如，生成的初始化选项可能如下：
                # {
                ...
                #         "resources": true
                #     },
                #     "instructions": "本服务器用于通过MCP协议访问MySQL数据库"
                # }
                # 这些信息会在 MCP 协议初始化阶段传递给客户端，帮助客户端了解服务器的能力、资源、工具等内容。
